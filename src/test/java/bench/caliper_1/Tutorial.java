...
public class Tutorial {
	 ...
	 * Notice:
	 * 
	 * - We write a class that extends com.google.caliper.Benchmark. - It
	 * contains a public instance method whose name begins with 'time' and which
	 * accepts a single 'int reps' parameter. - The body of the method simply
	 * executes the code we wish to measure, 'reps' times.
	 * 
	 * Example run:
	 * 
	 ...
	 * 
	 * Benchmark ns --------- --- NanoTime 233
	 */
	public static class Benchmark1 extends Benchmark {
		public void timeNanoTime(final int reps) {
	 ...
	 * Example run output:
	 * 
	 * Benchmark ns ----------------- --- NanoTime 248 CurrentTimeMillis 118
	 */
	public static class Benchmark2 extends Benchmark {
		...
		public void timeCurrentTimeMillis(final int reps) {
			for (int i = 0; i < reps; i++) {
	...
	public static class Benchmark3 extends Benchmark {
		...
		// IDEA tries to warn us!
		public void timeArrayIteration_BAD(final int reps) {
			for (int i = 0; i < reps; i++) {
				for (final int ignoreMe : array) {
				}
			}
		}
	}
	 ...
	 * In this case, we're indeed getting a bogus result. The problem is that
	 * the Java Virtual Machine is too smart: it detected the fact that the loop
	 * was producing no actual result, so it simply compiled it right out. The
	 * method never looped at all. To fix this, we need to use a dummy result
	 * value.
	 * 
	 * Notice:
	 * 
	 ...
	 * We're no longer timing *just* the code we want to be testing - our result
	 * will now be inflated by the (small) cost of addition. This is an
	 * unfortunate fact of life with microbenchmarking. In fact, we were already
	 * inflated by the cost of an int comparison, "i < reps" as it was.
	 * 
	...
	public static class Benchmark4 extends Benchmark {
		...
		public int timeArrayIteration_fixed(final int reps) {
			int dummy = 0;
			for (int i = 0; i < reps; i++) {
				for (final int doNotIgnoreMe : array) {
					dummy += doNotIgnoreMe;
				}
			}
			return dummy; // framework ignores this, but it has served its
							// purpose!
		}
	}
	 ...
	 * 
	 * When you run this benchmark the same way you ran the previous ones,
	 * you'll now get an error:
	 * "No values provided for benchmark parameter 'size'". You can provide the
	 * value requested at the command line like this:
	 * 
	 * [caliper_home]/caliper tutorial.Tutorial.Benchmark5 -Dsize=100}
	 * 
	 * You'll see output like this:
	 * 
	 * Benchmark size ns -------------- ---- --- ArrayIteration 100 51
	 * 
	 * Now that we've parameterized our benchmark, things are starting to get
	 * fun. Try passing '-Dsize=10,100,1000' and see what happens!
	 * 
	 * Benchmark size ns -------------- ---- -----------------------------------
	 * ArrayIteration 10 7 | ArrayIteration 100 49 |||| ArrayIteration 1000 477
	 * ||||||||||||||||||||||||||||||
	 */
	public static class Benchmark5 extends Benchmark {
		...
		public int timeArrayIteration(final int reps) {
			int dummy = 0;
			for (int i = 0; i < reps; i++) {
				for (final int doNotIgnoreMe : array) {
					dummy += doNotIgnoreMe;
